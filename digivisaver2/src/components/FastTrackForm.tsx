...
export default function FastTrackForm({ currency, onSuccess, onCancel, language = 'EN' }: FastTrackFormProps) {
  ...
  const getCalculatedFees = () => {
    const base = PACKAGE_RATES[formData.packageType];
  ...
  const validate = () => {
    ...
    if (!formData.arrivalDate) {
      ...
      if (formData.arrivalDate < todayStr) {
        ...
        const arrivalDateTimeStr = formData.arrivalTime 
          ...
          : `${formData.arrivalDate}T23:59:59`; // fallback if arrivalTime isn't entered yet
    ...
    if (!formData.contactName.trim()) freshErrors.contactName = isEn ? 'Lead passenger name is required' : 'Vui lòng nhập tên hành khách';
    ...
    } else if (!isValidInternationalPhone(formData.contactPhone)) {
    ...
    if (wantsInvoice) {
      if (!companyName.trim()) freshErrors.companyName = isEn ? 'Company registered name is required' : 'Vui lòng nhập tên công ty đăng ký';
      if (!taxCode.trim()) {
        freshErrors.taxCode = isEn ? 'Registered tax code (MST) is required' : 'Vui lòng nhập mã số thuế (MST)';
      ...
      if (!companyAddress.trim()) freshErrors.companyAddress = isEn ? 'Company tax billing address is required' : 'Vui lòng nhập địa chỉ trụ sở công ty';
            ...
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-md w-full text-center space-y-6 shadow-2xl">
              ...
              <div className="space-y-2">
                <h3 className="font-display font-extrabold text-lg">
                  {isEn ? 'Connecting to 9Pay Gate' : 'Đang kết nối tới Cổng 9Pay'}
                </h3>
          ...
          <HistoricalAutofill
            ...
            onSelect={(profile: HistoricalProfile) => {
              setFormData((prev) => ({
                ...
                contactPhone: formatPhoneE164(profile.phone),
              ...
              // If they opted for airport pickup or eSIMs from previous profiles, we can auto-enable
              ...
              <div 
                ...
                className={`p-4 rounded-2xl border cursor-pointer flex flex-col justify-between transition-all ${
                  paymentMethod === '9pay'
                    ...
                    : 'border-slate-250 hover:border-slate-350 bg-slate-50/40'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-slate-900 flex items-center">
                      ...
                      {isEn ? '9Pay Secured Wallet' : 'Ví Điện Tử 9Pay Bảo Mật'}
                    </span>
                    <span className="text-[9px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded uppercase">{isEn ? 'Default' : 'Mặc Định'}</span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-relaxed">
                    {isEn 
                      ? 'Authorize instantly via Credit/Debit card (Visa, Mastercard, JCB, Amex) through encrypted direct API pathways.' 
                      : 'Thanh toán trực tuyến tức thì bằng thẻ Tín dụng/Ghi nợ (Visa, Mastercard, JCB, Amex) qua cổng API mã hóa an toàn.'}
                  </p>
                </div>
              </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-slate-900 flex items-center">
                      ...
                      {isEn ? 'Domestic Bank Transfer (Napas, MoMo)' : 'Chuyển Khoản Ngân Hàng (Napas, MoMo)'}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-relaxed">
                    {isEn 
                      ? 'Local payment networks support. Show dynamic Napas QR code to scan and pay instantly with zero handling fees.' 
                      : 'Hỗ trợ tất cả ngân hàng Việt Nam. Hiển thị mã QR Napas quét trả ngay tức thì qua Mobile Banking, không mất phí giao dịch.'}
                  </p>
                </div>
              </div>
            </div>
