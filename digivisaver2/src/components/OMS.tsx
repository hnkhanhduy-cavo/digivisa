                ...
                <div className="flex flex-col justify-between items-start gap-4 border-b border-slate-100 pb-5">
                  ...
                  {partnerServiceTab === 'VAT' ? (
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      {(() => {
                        const currentStatusDisplay = normalizeStatusForTimeline(selectedOrder.status, selectedOrder.type);
                        return (
                          <div className="flex flex-col gap-2 w-full">
                            <div className={`border text-xs font-bold rounded-xl px-3 py-2.5 flex items-center justify-between w-full font-sans shadow-xs ${getStatusBadgeStyle(currentStatusDisplay)}`}>
                              <div className="flex items-center gap-2">
                                <span className="h-2 w-2 rounded-full bg-indigo-500 animate-pulse" />
                                <span>{currentStatusDisplay}</span>
                              </div>
                              <span className="text-[9.5px] bg-white/90 font-mono text-slate-600 px-2 py-0.5 rounded border border-slate-200/80 font-bold">
                                🔒 READ ONLY
                              </span>
                            </div>

                            {selectedOrder.type === 'Visa' && selectedOrder.subStatus && (
                              <div className="bg-slate-50 border border-slate-200 p-2.5 rounded-xl flex items-center justify-between text-xs">
                                <span className="text-[10px] text-slate-500 font-bold uppercase">Sub-Status:</span>
                                <span className="font-extrabold text-indigo-900 bg-indigo-50 border border-indigo-200 px-2.5 py-0.5 rounded text-[11px]">
                                  {selectedOrder.subStatus === 'Awaiting Paperwork' || selectedOrder.subStatus === 'More documents required'
                                    ? '⚠️ More Docs Required'
                                    : selectedOrder.subStatus === 'Standard Review' || selectedOrder.subStatus === 'Standard processing'
                                      ? 'Standard Review'
                                      : selectedOrder.subStatus === 'Approved'
                                        ? 'Approved & Issued'
                                        : selectedOrder.subStatus === 'Declined'
                                          ? 'Declined / Rejected'
                            ...
                            <div className="bg-indigo-50/70 border border-indigo-150 rounded-2xl p-3 space-y-2">
                              <p className="text-[11px] text-indigo-950 font-medium leading-relaxed flex items-start gap-1.5">
                                <span className="shrink-0">ℹ️</span>
                                <span>Status is synchronized live and managed exclusively through <strong>Agency Zalo / WA Sync</strong>.</span>
                              </p>
                              <button
                                type="button"
                                onClick={() => setOmsSubPage('agency_comms')}
                                className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                              >
                                <MessageSquare className="h-3.5 w-3.5" />
                                <span>Update Status in Agency Zalo / WA Sync</span>
                              </button>
                            </div>
                          </div>
                        );
