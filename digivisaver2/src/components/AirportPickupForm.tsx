...
import { 
  ...
  MapPin, ShieldAlert, ArrowRight, CreditCard, Sparkles, Smile, MessageSquare, FileText
} from 'lucide-react';
...
export default function AirportPickupForm({ currency, language, onSuccess, onCancel }: AirportPickupFormProps) {
  ...
  const getCalculatedFees = () => {
    const vPrice = getVehiclePrice(formData.airport || 'Tan Son Nhat (SGN)', formData.vehicleType);
  ...
  const validate = () => {
    ...
    } else if (!isValidInternationalPhone(formData.passengerPhone)) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderId = `DV-AP${Math.floor(10000 + Math.random() * 90000)}`;
    ...
    const newOrder: Order = {
      ...
      paymentStatus: paymentMethod === '9pay' ? 'Paid (9Pay)' : 'Paid (Bank Transfer)',
      details: finalBooking,
    };

    if (paymentMethod === '9pay') {
      setIsRedirecting(true);
      setTimeout(() => {
        safeOpen('https://9pay.vn', '_blank');
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        ...
        <form noValidate onSubmit={handleSubmit} className="p-6 sm:p-10 space-y-8">
          ...
          <HistoricalAutofill
            ...
            onSelect={(profile: HistoricalProfile) => {
              setFormData((prev) => ({
                ...prev,
                passengerName: `${profile.firstName} ${profile.lastName}`.trim(),
                passengerPhone: formatPhoneE164(profile.phone),
                passengerEmail: profile.email,
                vehicleType: profile.vehicleType,
                flightNumber: profile.flightNumber,
              }));
              setDestinationAddress(profile.destinationAddress);
              setPickupAddress(profile.pickupAddress);
              setOptionalNote(profile.specialRequests || '');
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {[
                ...
                const spec = FLEET_SPECS[car.id as AirportPickupBooking['vehicleType']];
              ...
              <div 
                ...
                className={`p-4 rounded-2xl border cursor-pointer flex flex-col justify-between transition-all ${
                  paymentMethod === '9pay'
                    ...
                    : 'border-slate-250 hover:border-slate-355 bg-slate-50/40'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-slate-900 flex items-center">
                      ...
                      {isEn ? '9Pay Secured Wallet' : 'Cổng Thanh Toán Bảo Mật 9Pay'}
                    </span>
                  ...
                  <p className="text-[11px] text-slate-505 leading-relaxed">
                    {isEn 
                      ...
                      : 'Thanh toán trực tuyến tức thì qua thẻ Quốc tế/Nội địa (Visa, Mastercard, JCB, Amex) qua hệ thống mã hóa bảo mật.'}
                  </p>
                </div>
              </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-slate-900 flex items-center">
                      ...
                      {isEn ? 'Domestic Bank Transfer (Napas, MoMo)' : 'Chuyển Khoản Ngân Hàng Nội Địa (Napas QR)'}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-505 leading-relaxed">
                    {isEn 
                      ? 'Local payment networks support. Show dynamic Napas QR code to scan and pay instantly with zero handling fees.' 
                      : 'Hỗ trợ mạng lưới ngân hàng Việt Nam. Hiển thị mã QR Napas động quét thanh toán ngay lập tức không mất phí phụ thu.'}
                  </p>
                </div>
              </div>
            </div>

            {/* Display 9Pay gateway details if selected */}
            {paymentMethod === '9pay' && (
              <motion.div 
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                className="p-5 bg-gradient-to-br from-indigo-950 to-slate-900 border border-indigo-800 text-white rounded-2xl space-y-3 shadow-lg"
              >
                  ...
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">
                        {isEn ? 'Encrypted API pathways for Visa, Mastercard, JCB & Amex' : 'Thanh toán trực tuyến bảo mật qua thẻ Visa, Mastercard, JCB & Amex'}
                      </p>
                    </div>
                  </div>
                ...
                <p className="text-[10px] text-indigo-300/80 italic pt-0.5">
                  {isEn 
                    ? 'ℹ️ Billing is processed in USD ($) via international card gateway. No hidden surcharges.' 
                    : 'ℹ️ Thanh toán được tính theo USD ($) qua cổng thẻ quốc tế. Không có phí ẩn.'}
                </p>
              </motion.div>
            )}
              ...
              >
                <div className="flex flex-col sm:flex-row gap-5 items-center">
                  <div className="p-2.5 bg-white rounded-xl shadow-lg shrink-0">
                    <img 
                      src={`https://img.vietqr.io/image/vcb-9999088888-compact2.png?amount=${fees.totalVnd}&addInfo=DVAP${formData.flightNumber || 'FLIGHT'}&accountName=DIGIVISA%20CO%20LTD`} 
                  ...
                  <div className="space-y-2 flex-1 text-xs">
                    <span className="text-[10px] font-mono font-bold uppercase text-teal-400 tracking-wider">
                      {isEn ? 'NAPAS QUICK TRANSFER' : 'CHUYỂN KHOẢN NHANH NAPAS'}
                    </span>
                    <div className="grid grid-cols-2 gap-y-1.5 gap-x-2 text-[11px] border-t border-slate-800 pt-2 text-slate-300">
                      ...
                      <div className="font-mono font-bold text-teal-400">DVAP{formData.flightNumber || 'FLIGHT'}</div>
                    </div>
                  </div>
                </div>
                <div className="text-[10px] text-slate-400 border-t border-slate-850 pt-2 font-sans">
                  {isEn 
                    ? 'Immediate automated verification is active. Transfer with exact description above. Chauffeur dispatch details sent within 10 minutes from approval.' 
                    : 'Hệ thống kiểm tra tự động đang hoạt động. Vui lòng chuyển khoản đúng số tiền và nội dung chính xác như trên. Thông tin tài xế sẽ được gửi trong vòng 10 phút sau khi giao dịch thành công.'}
                </div>
              </motion.div>
            )}
          </div>
