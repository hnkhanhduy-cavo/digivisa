...
export default function VisaForm({ language, currency, onSuccess, onCancel }: VisaFormProps) {
  ...
  // Real local passport / photo upload
  const simulateFileUpload = (file: File, type: 'passport' | 'photo') => {
    const isPassport = type === 'passport';
    const errorKey = isPassport ? 'passportScan' : 'photoScan';

    // 1. Check if it's an image file (strict photo restriction)
    const isImage = file.type.startsWith('image/') || /\.(jpg|jpeg|png|webp)$/i.test(file.name);
    if (!isImage) {
      setErrors((prev) => ({
        ...prev,
        [errorKey]: isEn 
          ? 'Only photo files (JPG, JPEG, PNG, WEBP) are strictly accepted.' 
          : 'Chỉ chấp nhận các tệp ảnh (JPG, JPEG, PNG, WEBP).'
      }));
      return;
    }

    // 2. Check file size limit (strict 5MB limit)
    const MAX_SIZE = 5 * 1024 * 1024; // 5MB
    if (file.size > MAX_SIZE) {
      setErrors((prev) => ({
        ...prev,
        [errorKey]: isEn 
          ? `File size exceeds the 5MB limit (your file is ${(file.size / (1024 * 1024)).toFixed(2)}MB). Please upload a smaller photo.` 
          : `Kích thước tệp vượt quá giới hạn 5MB (tệp của bạn là ${(file.size / (1024 * 1024)).toFixed(2)}MB). Vui lòng tải lên ảnh nhỏ hơn.`
      }));
      return;
    }

    if (isPassport) {
      setFormData((prev) => ({ ...prev, passportScan: file.name }));
      setErrors((prev) => {
        const copy = { ...prev };
      ...
      setFormData((prev) => ({ ...prev, photoScan: file.name }));
      setErrors((prev) => {
        const copy = { ...prev };
