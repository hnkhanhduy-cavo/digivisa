  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 text-slate-800 shadow-sm backdrop-blur-md border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-y-2 py-2 sm:py-0 sm:h-20">
          
          {/* Order 1: Logo Brand */}
          <div 
            className="order-1 flex items-center space-x-2.5 sm:space-x-3 cursor-pointer group animate-fade-in"
            onClick={() => setActiveTab('services')}
            id="header-brand-logo"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-600 rounded-lg flex items-center justify-center text-white shadow-sm shadow-indigo-600/10 group-hover:scale-105 transition-transform duration-200 shrink-0">
          ...
          {/* Order 2 on Mobile (Row 1 Right), Order 3 on Desktop: Right Action Bar */}
          <div className="order-2 sm:order-3 flex items-center space-x-1 sm:space-x-2.5">
            
            {/* Customer Login / Register Button */}
            {currentUser ? (
              <div className="flex items-center space-x-1 bg-indigo-50 border border-indigo-100 px-2 py-1 sm:px-3 sm:py-1.5 rounded-xl">
                <User className="h-3.5 w-3.5 text-indigo-600 shrink-0" />
                <span className="text-xs font-bold text-indigo-900 truncate max-w-[64px] sm:max-w-[140px]">
                  {currentUser.displayName || currentUser.email?.split('@')[0]}
                </span>
                {onLogout && (
                  <button
                    onClick={onLogout}
                    className="p-0.5 text-slate-400 hover:text-red-600 transition-colors shrink-0"
              ...
              <button
                ...
                className="flex items-center space-x-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs transition-colors shadow-xs cursor-pointer"
              >
            ...
            {userRole === 'staff' && (
              <button
                ...
                className="flex items-center space-x-1 px-2 py-1 sm:px-2.5 sm:py-1.5 bg-slate-800 hover:bg-slate-900 text-emerald-400 font-bold rounded-xl text-[10px] uppercase tracking-wider transition-colors cursor-pointer border border-slate-700"
            ...
            <div className="flex items-center bg-slate-50 border border-slate-200 hover:border-slate-300 transition-colors px-1.5 py-1 sm:px-2.5 sm:py-1.5 rounded-xl shadow-xs">
          ...
          {/* Order 3 on Mobile (Row 2), Order 2 on Desktop: Navigation Links */}
          <nav className="order-3 sm:order-2 w-full sm:w-auto flex items-center justify-around sm:justify-start space-x-1 bg-slate-50 p-1 rounded-xl border border-slate-150/80">
            <button
              id="nav-services-btn"
              title={t.services}
              onClick={() => setActiveTab('services')}
              className={`flex-1 sm:flex-initial flex items-center justify-center space-x-1 sm:space-x-1.5 px-2 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 cursor-pointer ${
                activeTab === 'services'
            ...
            <button
              ...
              className={`flex-1 sm:flex-initial relative flex items-center justify-center space-x-1 sm:space-x-1.5 px-2 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 cursor-pointer ${
                activeTab === 'tracker'
                  ? 'bg-white text-indigo-600 border border-slate-200/80 shadow-sm font-semibold'
                  : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100/50 border border-transparent'
              }`}
            ...
            >
              ...
              <span className="hidden sm:inline">{t.guideInfo}</span>
            </button>
            ...
            {userRole === 'staff' && (
              ...
              >
                ...
                <span className="hidden sm:inline">{t.staffOms}</span>
